...
const StorePage = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const response = await fetch(`https://talabatak-backend2.vercel.app/api/categories`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const handleToggleFavorite = async (product: APIProduct) => {
    ...
    if (token) {
      try {
        await fetch(`https://talabatak-backend2.vercel.app/api/favorites/${product._id}`, {
